            ...
            registry.addHandler(
                new TextWebSocketHandler()
                {
                    ...
                    public void afterConnectionClosed(WebSocketSession session, CloseStatus status)
                    {
                        ...
                        this.sessions.values().stream().forEach($ -> $.sendMessage(new TextMessage(objectMapper.writeValueAsString(java.util.Map.ofEntries(java.util.Map.entry("action", "disconnect"), java.util.Map.entry("name", session.getAttributes().get("name")))))));
                    }
                    @Override
                    protected void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception
                    {
                        if (!this.sessions.containsKey(session.getId()))
                        {
                            ...
                            this.sessions.values().stream().forEach($ -> $.sendMessage(new TextMessage(objectMapper.writeValueAsString(java.util.Map.ofEntries(java.util.Map.entry("action", "join"), java.util.Map.entry("name", session.getAttributes().get("name")))))));
