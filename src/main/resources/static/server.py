...
async def database(app):
    app.setdefault('database', await asyncpg.create_pool(os.getenv('DATABASE_URL') if 'DATABASE_URL' in os.environ else ''.join(('postgres://postgresql:postgresql@', os.getenv('AIOHTTPPOSTGRE_SERVICE_HOST'), ':', os.getenv('AIOHTTPPOSTGRE_SERVICE_PORT'), '/sampledb'))))  
