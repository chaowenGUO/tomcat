...
async def database(app):
    app.setdefault('database', await asyncpg.create_pool(os.getenv('DATABASE_URL')) if 'DATABASE_URL' in os.environ else await asyncpg.create_pool(''.join(('postgres://', os.getenv('POSTGRESQL_USER'), ':', os.getenv('POSTGRESQL_PASSWORD'), '@', os.getenv('POSTGRESQL_SERVICE_HOST'), ':', os.getenv('POSTGRESQL_SERVICE_PORT'), '/', os.getenv('POSTGRESQL_DATABASE')))))
