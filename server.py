...
async def database(app):
    app.setdefault('database', await asyncpg.create_pool(os.getenv('DATABASE_URL') if 'DATABASE_URL' in os.environ else ''.join(('postgres://postgre:postgre@', os.getenv('POSTGRESQL_SERVICE_HOST'), ':', os.getenv('POSTGRESQL_SERVICE_PORT'), '/sampledb'))))  
