...
async def websocket(app):
    app.setdefault('websocket', {})
    yield
    for _ in app.get('websocket').values(): await _.close()
    
async def chat(request):
    websocket = web.WebSocketResponse()
    await websocket.prepare(request)
    name = await websocket.receive_str()
    for _ in request.app.get('websocket').values(): await _.send_str(json.dumps({'action':'join', 'name':name}))
    request.app.get('websocket').setdefault(name, websocket)
    async for message in websocket:
        if message.type == WSMsgType.TEXT:
            for _ in request.app.get('websocket').values():
                if _ is not websocket: await _.send_str(json.dumps({'action':'sent', 'name': name, 'text':message.data}))
        else: break
    request.app.get('websocket').pop(name)
    for _ in request.app.get('websocket').values(): await _.send_str(json.dumps({'action':'disconnect', 'name':name}))
    return websocket
